...
def stereo():
    with reip.Task("Cam_Task_0"):
        cam_0 = UsbCamGStreamer(name="Cam_0", filename=DATA_DIR + "/video/%d_{time}.avi", dev=0,
                              bundle=None, rate=3, debug=True, verbose=False)
...
def all(audio_length=10):
    with reip.Task("Cam_Task_0"):
        cam_0 = UsbCamGStreamer(name="Cam_0", filename=DATA_DIR + "/video/%d_{time}.avi", dev=0,
                              bundle=None, rate=1, debug=True, verbose=False)
    ...
    with reip.Task("Mic_Array_Task"):
        ...
        audio_ns = audio1s.to(B.Rebuffer(duration=audio_length))

    with reip.Task("Audio_Writer_Task"):
        audio_ns.to(B.audio.AudioFile(os.path.join(DATA_DIR, 'audio/{time}.wav')), throughput='large')
